...
function renderCommerce(){
  ...
  campaigns.innerHTML=commerceState.campaigns.length?commerceState.campaigns.map(campaign=>`<article class="commerce-campaign"><div class="commerce-campaign-head"><div><span class="tag">${esc(commerceStatus[campaign.status]||campaign.status)}</span><h4>${esc(campaign.name)}</h4><p>${esc(campaign.product_title)} · ${esc(campaign.destination_label)}${campaign.pinterest_status?' · Video Pin pronto':''}</p></div><b>${money(campaign.commission)}</b></div><div class="commerce-kpis"><span><b>${Number(campaign.clicks).toLocaleString('pt-BR')}</b> cliques</span><span><b>${Number(campaign.conversions).toLocaleString('pt-BR')}</b> conversões</span><span><b>${campaign.roi===null?'—':campaign.roi+'%'}</b> ROI</span></div><div class="commerce-campaign-actions"><button class="secondary" data-commerce-metrics="${esc(campaign.id)}">Registrar resultado</button>${campaign.job_id?`<button class="primary" data-commerce-package="${esc(campaign.id)}" ${campaign.status==='packaged'?'disabled':''}>${campaign.status==='packaged'?'Pacote Shopee pronto':'Preparar Shopee'}</button><button class="pinterest-action" data-pinterest-package="${esc(campaign.id)}" ${campaign.pinterest_status==='prepared'?'disabled':''}>${campaign.pinterest_status==='prepared'?'Video Pin pronto':'Criar Video Pin'}</button>`:'<button class="secondary" disabled>Vincule uma produção</button><button class="pinterest-action" disabled>Video Pin indisponível</button>'}</div></article>`).join(''):'<div class="commerce-empty">Crie uma campanha depois de validar um produto.</div>';
...
document.querySelector('#commerce-campaigns').addEventListener('click',async event=>{
  ...
  const pinterestButton=event.target.closest('[data-pinterest-package]');
  if(pinterestButton){pinterestButton.disabled=true;try{await api(`/api/commerce-center/campaigns/${encodeURIComponent(pinterestButton.dataset.pinterestPackage)}/pinterest-package`,{method:'POST',body:JSON.stringify({duration:30,board_name:'Achados úteis'}),timeoutMs:120000});toast('Video Pin preparado. O login ainda será necessário para enviar.','success');await loadCommerce()}catch(error){toast(error.message,'error')}finally{pinterestButton.disabled=false}}
});
