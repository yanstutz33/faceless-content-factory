...
const isLegacyProduction=job=>['rejected','failed'].includes(job.status)&&legacyFailurePattern.test(`${job.error||''} ${job.metadata?.music?.style||''}`);
...
async function jobAction(id,action){let body={};if(action==='reject'){const reason=await requestRevisionReason();if(reason===null)return;body={reason}}try{await api(`/api/jobs/${encodeURIComponent(id)}/${action}`,{method:'POST',body:JSON.stringify(body)});$('#job-dialog').close();toast(action==='approve'?'Pacote aprovado para envio manual.':'Produção atualizada.','success');await load();if(['approve','reject'].includes(action)&&typeof window.guidedReviewAdvance==='function')await window.guidedReviewAdvance(id)}catch(error){toast(error.message,'error')}}
...
$('#production-new').onclick=()=>$('#create-dialog').showModal();
...
async function loadFlowMusicGuide(){
  ...
  try{
    ...
    root.innerHTML=`<div class="flow-guide-head"><div><p class="kicker">GOOGLE MUSIC · FLOW + LYRIA 3</p><h3>Geração direta ou importação do Flow</h3><p>A API oficial do Lyria cria e baixa o catálogo dentro do Hub. O Flow continua disponível para usar os créditos da assinatura.</p></div><div class="flow-actions">${connected?'<button class="primary" id="lyria-batch" type="button">✦ Gerar lote diverso</button>':'<button class="primary" id="lyria-connect" type="button">Conectar geração direta</button>'}<a class="secondary flow-launch" href="https://www.flowmusic.app/" target="_blank" rel="noopener noreferrer">Abrir Flow ↗</a><button class="secondary" id="flow-import-music" type="button">＋ Importar downloads</button></div></div><div class="lyria-cost"><b>${connected?'✓ Lyria 3 conectado':'API direta ainda desconectada'}</b><span>Pro · US$ 0,08 por música</span><span>Lote diverso de 12 · US$ 0,96</span><span>Revisão auditiva obrigatória</span></div><details class="flow-workflow"><summary>Entender as duas opções</summary><ol>${(data.workflow||[]).map(step=>`<li>${esc(step)}</li>`).join('')}</ol></details><details><summary>Ver as 12 direções musicais</summary><div class="flow-prompt-grid">${(data.prompts||[]).map(item=>`<article><div><b>${esc(item.name)}</b><span><button class="secondary" type="button" data-copy-flow="${esc(item.prompt)}">Copiar</button>${connected?`<button class="primary" type="button" data-generate-flow="${esc(item.prompt)}" data-flow-name="${esc(item.name)}">Gerar esta</button>`:''}</span></div><p>${esc(item.prompt)}</p></article>`).join('')}</div><div class="api-alternative">${connected?`<b>Gemini API conectada.</b><button class="secondary" id="lyria-generate" type="button">Criar uma faixa livre</button>${data.api.key_source==='encrypted_vault'?'<button class="secondary" id="lyria-disconnect" type="button">Desconectar API</button>':''}`:'<b>A conexão é feita uma vez e a chave fica criptografada neste computador.</b>'}</div></details>`;
    $('#flow-import-music').onclick=()=>$('#music-dialog').showModal();
    $('#lyria-connect')?.addEventListener('click',()=>$('#lyria-key-dialog').showModal());
    $('#lyria-batch')?.addEventListener('click',()=>{$('#lyria-batch-error').textContent='';$('#lyria-batch-dialog').showModal()});
...
$('#lyria-batch-form').addEventListener('submit',async event=>{
  event.preventDefault();const form=event.currentTarget;const fields=new FormData(form);const error=$('#lyria-batch-error');const button=event.submitter;error.textContent='';button.disabled=true;button.textContent='Gerando no Google… pode levar alguns minutos';
  try{const result=await api('/api/music-sources/lyria/generate-batch',{method:'POST',timeoutMs:3600000,body:JSON.stringify({count:Number(fields.get('count')),model:fields.get('model'),rights_confirmed:!!fields.get('rights_confirmed')})});form.reset();$('#lyria-batch-dialog').close();apiCache.clear();await Promise.all([loadMusicAssets(),loadCatalogReadiness()]);toast(`${result.generated} de ${result.requested} músicas geradas e aguardando sua audição.${result.failed?` ${result.failed} falharam e podem ser tentadas novamente.`:''}`,'success')}catch(problem){error.textContent=problem.message}finally{button.disabled=false;button.textContent='Gerar lote automaticamente'}
});
