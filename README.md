...
O painel **Flow Music Bridge** liga o hub ao produtor oficial em uma aba protegida pelo Google e mantém geração, prompts, downloads e importação como um único fluxo operacional. O Flow Music bloqueia incorporação em sites terceiros e exige autorização própria, portanto o hub não simula um login interno. O caminho recomendado usa os créditos do Google AI Plus/Flow Music Starter: gerar, baixar e importar o lote. A interface também conserva a integração opcional com Lyria 3 pela Gemini API; a chave é guardada no cofre criptografado local, nunca retorna ao navegador e a cobrança da API é separada da assinatura. As 12 direções musicais foram recalibradas para lo-fi melancólico, lento e pouco percussivo.
...
O kit visual público do canal fica em `assets/channel` e também aparece na Biblioteca com botões de download. Ele usa a identidade existente de **Pausa Pra Anime** (`@pausapraanime`), uma cena anime original e a assinatura discreta da **YAMI**. Inclui avatar 800×800, banner 2560×1440 com conteúdo dentro da área segura central e marca-d'água transparente. O FFactory permanece como infraestrutura interna. `python scripts/generate_channel_brand.py` recompõe o kit de forma determinística a partir dos arquivos-fonte registrados.
